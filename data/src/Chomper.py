...
class Chomper(Object):  # 定义nut类，继承自Object类
    ...
    def run(self):  # 运行函数
        if self.state == "Idle" and self.imageIndex == self.imageCount and not self.eat:  # 如果状态为Eat（进食）
            ...
            self.eat = True  # 设置状态为True（进食）

        elif self.state == "Eat" and self.imageIndex == self.imageCount and self.eat:  # 如果状态为Eat（进食）
